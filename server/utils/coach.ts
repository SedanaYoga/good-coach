import { getAthleteConfig, getActivities, getWorkoutsPlan, updateWorkout, getDb } from './db';
...
export async function matchAndAnalyzeActivities() {
  ...
  for (const act of unmatchedActivities) {
    ...
    for (const w of candidateWorkouts) {
      ...
      const isRunActivity = act.sport_type === 'Run';
      const isRunWorkout = ['easy_run', 'long_run', 'interval'].includes(w.workout_type);
      const isStrengthActivity = ['WeightTraining', 'Workout', 'Strength', 'Gym'].includes(act.sport_type);
      const isStrengthWorkout = w.workout_type === 'strength';
      
      if ((isRunActivity && isRunWorkout) || (isStrengthActivity && isStrengthWorkout)) {
        matchedWorkout = w;
        break;
      }
    }
    ...
    if (matchedWorkout) {
      console.log(`Coach: Matching activity ${act.id} ("${act.name}") with workout ${matchedWorkout.id} ("${matchedWorkout.title}")`);
      ...
      db.prepare('UPDATE activities SET matched_workout_id = ?, coach_feedback = ? WHERE id = ?')
...
export function getWorkoutDate(raceDateStr: string, totalWeeks: number, weekNumber: number, dayNumber: number): string {
